...
struct SOpportunityCandidate
  {
   ...
   double                     zoneLow;
   double                     zoneHigh;
   double                     triggerPrice;    // P1 Révision 3 (V4.1-P3.1bis) - prix EXACT (ask ou bid selon direction) au moment de la transition CREATED->TRIGGERED. 0.0 tant que non déclenché. Nécessaire pour VirtualTradeFeed (calcul SL/TP via CRiskManager) - EvaluatePrice() connaissait déjà ce prix mais ne le conservait pas.
  };
