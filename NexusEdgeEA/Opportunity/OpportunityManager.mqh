...
public:
   ...
   // sur un identifiant (demande explicite §6 de la spec). Fait
   // transitionner en TRIGGERED tout candidat CREATED du symbole donné
   // dont la zone contient le prix de déclenchement PERTINENT pour SA
   // PROPRE direction.
   //
   // P1 RÉVISION 2 (V4.1-P3, intégration réelle) : reçoit désormais
   // bid ET ask - le CHOIX entre les deux (BUY -> ask, SELL -> bid,
   // même convention que le reste du moteur pour "entryPrice") est
   // fait ICI, à l'intérieur du manager, jamais par l'orchestrateur.
   // Justification (revue P3) : "le choix Bid/Ask fait partie de la
   // logique métier de déclenchement, pas de l'orchestration - sinon
   // le .mq5 commencerait à connaître les règles internes
   // d'OpportunityManager." Ne lit jamais le marché elle-même :
   // bid/ask sont reçus en paramètre (invariant 4).
   //---------------------------------------------------------------
   int               EvaluatePrice(const string symbol, const double bid, const double ask)
     {
      ...
      for(int i = 0; i < total; i++)
        {
         ...
         double triggerPrice = (m_candidates[i].direction == OPPORTUNITY_DIRECTION_BUY) ? ask : bid;
         if(!COpportunityCandidateFactory::PriceInZone(m_candidates[i], triggerPrice))
            continue;

         m_candidates[i].state        = OPPORTUNITY_STATE_TRIGGERED;
         m_candidates[i].triggerPrice = triggerPrice; // P1 Révision 3
         triggeredCount++;
        }
